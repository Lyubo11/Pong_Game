...
// DrawPaddle a line in 3D world space
...
// DrawPaddle a point in 3D space, actually a small line
...
// DrawPaddle a circle in 3D world space
...
// DrawPaddle a color-filled triangle (vertex in counter-clockwise order!)
...
// DrawPaddle cube (Vector version)
void DrawCubeV(Vector3 position, Vector3 size, Color color)
{
    DrawCube(position, size.x, size.y, size.z, color);
}

// DrawPaddle cube wires
...
// DrawPaddle cube wires (vector version)
...
// DrawPaddle sphere with extended parameters
...
// DrawPaddle a cylinder with base at startPos and top at endPos
// NOTE: It could be also used for pyramid and cone
...
// DrawPaddle a wired cylinder with base at startPos and top at endPos
// NOTE: It could be also used for pyramid and cone
...
// DrawPaddle a capsule with the center of its sphere caps at startPos and endPos
...
// DrawPaddle capsule wires with the center of its sphere caps at startPos and endPos
...
// DrawPaddle a grid centered at (0, 0, 0)
...
// UpdatePlayerPaddle mesh vertex data in GPU for a specific buffer index
void UpdateMeshBuffer(Mesh mesh, int index, const void *data, int dataSize, int offset)
{
    rlUpdateVertexBuffer(mesh.vboId[index], data, dataSize, offset);
}

// DrawPaddle a 3d mesh with material and transform
...
// DrawPaddle multiple mesh instances with material and different transforms
...
// UpdatePlayerPaddle model animated vertex data (positions and normals) for a given frame
...
void UpdateModelAnimation(Model model, ModelAnimation anim, int frame)
{
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        ...
        for (int m = 0; m < model.meshCount; m++)
        {
            ...
            if (updated)
            {
                rlUpdateVertexBuffer(mesh.vboId[0], mesh.animVertices, mesh.vertexCount*3*sizeof(float), 0); // UpdatePlayerPaddle vertex position
                rlUpdateVertexBuffer(mesh.vboId[2], mesh.animNormals, mesh.vertexCount*3*sizeof(float), 0);  // UpdatePlayerPaddle vertex normals
            }
        }
    }
}
...
void GenMeshTangents(Mesh *mesh)
{
    ...
    if (mesh->vboId != NULL)
    {
        ...
        {
            // UpdatePlayerPaddle existing vertex buffer
...
// DrawPaddle a model (with texture if set)
...
// DrawPaddle a model with extended parameters
...
// DrawPaddle a model wires (with texture if set)
...
// DrawPaddle a model wires (with texture if set) with extended parameters
...
// DrawPaddle a billboard (part of a texture defined by a rectangle)
...
// DrawPaddle a bounding box with wires

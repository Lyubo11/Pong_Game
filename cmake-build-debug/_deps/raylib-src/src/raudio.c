...
// UpdatePlayerPaddle sound buffer with new data
...
// UpdatePlayerPaddle (re-fill) music buffers if data already processed
...
// UpdatePlayerPaddle audio stream buffers with data
    ...
    if (stream.buffer != NULL)
    {
        if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1])
        {
            ...
            if (stream.buffer->isSubBufferProcessed[0] && stream.buffer->isSubBufferProcessed[1])
            {
                ...
                // UpdatePlayerPaddle the first one and make sure the cursor is moved back to the front.
                subBufferToUpdate = 0;

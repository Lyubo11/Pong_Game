...
Font LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *codepoints, int codepointCount)
{
        ...
        TextIsEqual(fileExtLower, ".otf"))
    {
        ...
        if (font.glyphs != NULL)
        {
            ...
            // UpdatePlayerPaddle glyphs[i].image to use alpha, required to be used on ImageDrawText()
            for (int i = 0; i < font.glyphCount; i++)
            {
...
// DrawPaddle text (using default font)
...
// DrawPaddle text using Font and pro parameters (rotation)
...
// DrawPaddle one character (codepoint)
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    ...
    // DrawPaddle the character texture on the screen
    DrawTexturePro(font.texture, srcRec, dstRec, (Vector2){ 0, 0 }, 0.0f, tint);
}

// DrawPaddle multiple character (codepoints)

...
// DrawPaddle a pixel (Vector version)
...
// DrawPaddle a line (using gl lines)
...
// DrawPaddle lines sequuence (using gl lines)
...
// DrawPaddle line using cubic-bezier spline, in-out interpolation, no control points
...
// DrawPaddle a line defining thickness
...
// DrawPaddle a color-filled circle
void DrawCircle(int centerX, int centerY, float radius, Color color)
{
    DrawCircleV((Vector2){ (float)centerX, (float)centerY }, radius, color);
}

// DrawPaddle a color-filled circle (Vector version)
...
// DrawPaddle a piece of a circle
...
// DrawPaddle a piece of a circle outlines
...
// DrawPaddle a gradient-filled circle
...
// DrawPaddle a color-filled rectangle with pro parameters
...
// DrawPaddle a vertical-gradient-filled rectangle
...
// DrawPaddle a horizontal-gradient-filled rectangle
// NOTE: Gradient goes from bottom (color1) to top (color2)
...
// DrawPaddle a gradient-filled rectangle
...
// DrawPaddle rectangle outline with extended parameters
...
// DrawPaddle rectangle with rounded edges
void DrawRectangleRounded(Rectangle rec, float roundness, int segments, Color color)
{
    ...
    */
    ...
    rlBegin(RL_QUADS);
        // DrawPaddle all the 4 corners: [1] Upper Left Corner, [3] Upper Right Corner, [5] Lower Right Corner, [7] Lower Left Corner
    ...
    rlBegin(RL_TRIANGLES);
        ...
        // DrawPaddle all of the 4 corners: [1] Upper Left Corner, [3] Upper Right Corner, [5] Lower Right Corner, [7] Lower Left Corner
        for (int k = 0; k < 4; ++k) // Hope the compiler is smart enough to unroll this loop
        {
            float angle = angles[k];
            const Vector2 center = centers[k];
...
// DrawPaddle rectangle with rounded edges outline
    ...
    */
    ...
    if (lineThick > 1)
    {
        ...
        rlBegin(RL_QUADS);
            ...
            // DrawPaddle all the 4 corners first: Upper Left Corner, Upper Right Corner, Lower Right Corner, Lower Left Corner
            for (int k = 0; k < 4; ++k) // Hope the compiler is smart enough to unroll this loop
            {
                float angle = angles[k];
                const Vector2 center = centers[k];
        ...
        rlBegin(RL_TRIANGLES);
            ...
            // DrawPaddle all of the 4 corners first: Upper Left Corner, Upper Right Corner, Lower Right Corner, Lower Left Corner
            for (int k = 0; k < 4; ++k) // Hope the compiler is smart enough to unroll this loop
            {
                float angle = angles[k];
                const Vector2 center = centers[k];
...
// DrawPaddle a triangle using lines
// NOTE: Vertex must be provided in counter-clockwise order
...
// DrawPaddle a triangle fan defined by points
...
// DrawPaddle a triangle strip defined by points
...
// DrawPaddle a regular polygon of n sides (Vector version)
...
// DrawPaddle a polygon outline of n sides
...
// DrawPaddle spline: linear, minimum 2 points
...
// DrawPaddle spline: B-Spline, minimum 4 points
void DrawSplineBasis(Vector2 *points, int pointCount, float thick, Color color)
{
    ...
    for (int i = 0; i < (pointCount - 3); i++)
    {
        ...
        if (i == 0) DrawCircleV(currentPoint, thick/2.0f, color);   // DrawPaddle init line circle-cap
    ...
    DrawCircleV(currentPoint, thick/2.0f, color);   // DrawPaddle end line circle-cap
}

// DrawPaddle spline: Catmull-Rom, minimum 4 points
void DrawSplineCatmullRom(Vector2 *points, int pointCount, float thick, Color color)
{
    ...
    DrawCircleV(currentPoint, thick/2.0f, color);   // DrawPaddle init line circle-cap
...
// DrawPaddle spline: Quadratic Bezier, minimum 3 points (1 control point): [p1, c2, p3, c4...]
...
// DrawPaddle spline: Cubic Bezier, minimum 4 points (2 control points): [p1, c2, c3, p4, c5, c6...]
...
// DrawPaddle spline segment: Linear, 2 points
...
// DrawPaddle spline segment: B-Spline, 4 points
...
// DrawPaddle spline segment: Catmull-Rom, 4 points
...
// DrawPaddle spline segment: Quadratic Bezier, 2 points, 1 control point
...
// DrawPaddle spline segment: Cubic Bezier, 2 points, 2 control points
